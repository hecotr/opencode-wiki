...
- 登录 **[OpenCode Zen](https://opencode.ai/auth)**，订阅 Go，然后复制你的 API 密钥。

- 在 TUI 中运行 `/connect` 命令，选择 `OpenCode Go`，然后粘贴你的 API 密钥。

- 在 TUI 中运行 `/models` 以查看通过 Go 可用的模型列表。
...
- **无锁定（no lock-in）**，允许你与 OpenCode 一起使用任何其他提供商。
